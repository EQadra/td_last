...
export default function LawyerDetailScreen() {
  ...
  const { user } = useAuth();
  const { lawyer, fetchLawyerById, loading, error } = useLawyers();
  ...
  // ✅ VERIFICAR SI EL USUARIO ES DUEÑO DEL ABOGADO
  useEffect(() => {
    if (user && lawyer) {
      setIsOwner(user.id === lawyer.user_id);
    }
  }, [user, lawyer]);
  ...
  // ============================================================
  // ✅ FUNCIONES PARA CREAR POST - COPIADAS DE PostsScreen
  // ============================================================
  ...
  const handleCreatePost = async () => {
    ...
    if (!lawyer || !lawyer.id) {
      Alert.alert("Error", "No se encontró el abogado");
      return;
    }
    ...
    try {
      const payload: any = {
        ...
        postable_type: 'App\\Models\\Lawyer',
        postable_id: lawyer.id,
      };
      ...
      console.log('📤 Enviando post desde LawyerDetail:', {
        title: payload.title,
  ...
  // ============================================================
  // ✅ FUNCIONES PARA CREAR SERVICIO - COPIADAS DE ServicesScreen
  // ============================================================
  ...
  // ✅ HANDLE CREATE SERVICE - CON IMAGEN
  const handleCreateService = async () => {
    ...
    if (!profile || !profile.id) {
      Alert.alert("Error", "No se encontró el perfil del abogado");
      return;
    }
    ...
    try {
      ...
      console.log('📤 Enviando servicio desde LawyerDetail:', {
        name: payload.name,
        hasImage: !!payload.image,
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.headerCard}>
          <View style={styles.imageContainer}>
            <Image
              source={{
                uri: lawyer.image || "https://tudealer.app/avatar_lawyer.jpg",
              }}
        ...
        {activeTab === "sobre" && (
          <View style={styles.card}>
            ...
            <View style={styles.ratingContainer}>
              ...
              <Text style={styles.ratingCount}>({lawyer.feedbacks?.length || 0} reseñas)</Text>
            </View>
          </View>
        )}
            ...
            {lawyer.services && lawyer.services.length > 0 ? (
              lawyer.services.map((s) => (
                <View key={s.id} style={styles.card}>
                  {s.image && <Image source={{ uri: s.image }} style={styles.postImage} resizeMode="cover" />}
      ...
      {/* ✅ MODAL - CREAR POST (COPIADO DE PostsScreen) */}
      <Modal
      ...
      {/* ✅ MODAL - CREAR SERVICIO (COPIADO DE ServicesScreen) */}
      <Modal
            ...
            >
              <View style={[styles.modalContentLarge, styles.modalContentService]}>
                ...
                <View style={styles.modalActions}>
                  ...
                  >
                    <Text style={styles.publishBtnText}>
                      {isSubmittingService ? "Creando..." : "Crear"}
                    </Text>
                  </TouchableOpacity>
                </View>
              </View>
